
    <footer class="main-footer">
        <div class="pull-right hidden-xs">
            Developed By: <a href="#"><b>DDP System Solutions</b></a>
        </div>
        <strong>Copyright &copy; <?=date("Y");?> <a href="#">ACQ Solomonic Builders Development Corp</a>.</strong> All rights reserved.
    </footer>

    <?php include("includes/control_sidebar.php");?>
    <div class="control-sidebar-bg"></div>
</div>

<script src="resources/theme/bower_components/jquery/dist/jquery.min.js"></script>
<script src="resources/theme/bower_components/jquery/dist/jquery.form.min.js"></script>
<script src="resources/theme/bower_components/bootstrap/dist/js/bootstrap.min.js"></script>
<script src="resources/theme/dist/js/adminlte.min.js"></script>


<script src="resources/theme/bower_components/moment/min/moment.min.js"></script>
<script src="resources/theme/bower_components/bootstrap-daterangepicker/daterangepicker.js"></script>

<script src="resources/js/gen-script.js"></script>
</body>
</html>